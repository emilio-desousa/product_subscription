...
class MarketingCampaign:
    ...
    def __init__(
        ...
        """
        self.features_to_binary_convert = features_to_binary_convert
    ...
    def _clean_accents(self, df, columns_to_clean_accents=[stg.COL_RAW_JOB, stg.COL_RAW_STATUS]):
        df_without_accent = df.copy()
        for col in columns_to_clean_accents:
            df_without_accent[col] = df[col].str.normalize("NFKD").str.encode("ascii", errors="ignore").str.decode("utf-8")
        return df_without_accent

    def _delete_outliers_from_age(self, df):
        df.loc[df[stg.COL_RAW_AGE] > 110, stg.COL_RAW_AGE] = df[stg.COL_RAW_AGE].median()
        return df

    def _convert_to_cat_type(self, df):
        return df.astype(stg.COLS_TO_CAT_CONVERT)

    def _easy_yes_no_converter(self, df):
        df_yes_no = df.copy()
        for col in self.features_to_binary_convert:
