...
class DatasetBuilder:
    ...
    def __init__(self, filename_bank, filename_socio, is_test=False):
        """initialise class
        ...
        """
        if is_test:
            features_to_binary_convert = [
                stg.COL_RAW_HAS_HOUSING_LOAN,
                stg.COL_RAW_HAS_PERSO_LOAN,
                stg.COL_RAW_HAS_DEFAULT,
            ]
            self.dataset_marketing = MarketingCampaign(
                filename=filename_bank,
                features_to_binary_convert=features_to_binary_convert,
            ).data
        else:
    ...
    def create_dataset(self):
        ...
        dataset_with_month_and_weekday = self._add_month_weekday(dataset_with_datetime_date)
    ...
    def _col_date_to_datetime(self, df):
        return df.assign(**{stg.COL_RAW_DATE: lambda df: pd.to_datetime(df[stg.COL_RAW_DATE])})
    ...
    def _add_year_month(self, df):
        ...
        df_with_year_month = df.assign(
            **{stg.COL_YEAR_MONTH: lambda df: dataset_with_datetime_date[stg.COL_RAW_DATE].dt.to_period("M")}
        )
